				...
				$http.get(appConfig.dashboardServiceUrl + 'api_colors/search_rgb', {params: RGB})
					.then(function (res) {
						if (res.data.rgb) {
							...
							for (var i = 0; similarSaturateColors.length <= 50; ++i) {
								console.log('chroma(255, 51, 153).saturate(0.1 * i).rgb()', chroma(255, 51, 153).saturate(0.4 * i).rgb());
								rgbChroma = chroma(255, 51, 153).saturate(0.4 * i).rgb();
								similarSaturateColors.push(chroma(255, 51, 153).saturate(0.1 * i).rgb());
								similarDarkenColors.push(chroma(255, 51, 153).darken(0.05 * i).rgb());
							}
							var arr = [1,2,3,4,5];

							for (var i = 0; arr.length <= 10; ++i)  {
								RGB.red = parseInt(RGB.red) + i;
								RGB.green = parseInt(RGB.green) + i;
								RGB.blue = parseInt(RGB.blue) + i;

								arr.push(i)
								console.log('RGB', RGB);
							}
							console.log('similarDarkenColors', similarDarkenColors);
